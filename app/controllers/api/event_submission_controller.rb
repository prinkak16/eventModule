class Api::EventSubmissionController < Api::ApplicationController
  ...
  def user_submissions
    ...
    submissions = EventSubmission.where(user_id: current_user.id, event_id: params[:event_id])
  ...
  def user_submit_event
    ...
    data = { eventId: event.id, formId: event_form.form_id, submissionId: submission.submission_id, eventName: event.name, eventStartDate: event.start_date, isFormCreator: false, eventEndDate: event.end_date, user: { name: current_user.name }, dataLevel: event.data_level&.name, eventStateIds: event.event_locations.pluck(:state_id) }
